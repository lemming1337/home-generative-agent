...
async def _download_marked(hass: HomeAssistant) -> bool:
    ...
    try:
        ...
        # Save version information
        async with aiofiles.open(version_file, "w") as f:
            await f.write(f"{MARKED_VERSION}\n{sha256_hash}")
...
async def _ensure_marked_available(hass: HomeAssistant) -> bool:
    ...
    cached_exists = await hass.async_add_executor_job(cached_file.exists)
    if not cached_exists:
        LOGGER.info("marked.js not found in cache, will download")
        needs_download = True
    else:
        version_exists = await hass.async_add_executor_job(version_file.exists)
        if not version_exists:
            ...
            try:
                async with aiofiles.open(version_file, "r") as f:
                    cached_version_text = await f.read()
                cached_version = cached_version_text.strip().split("\n")[0]
    ...
    return await hass.async_add_executor_job(cached_file.exists)
